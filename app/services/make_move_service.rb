class MakeMoveService
  ...
  def call
    ...
    if !PlayerTurnPolicy.new(round: @player.game.current_round).turn_of?(@player)
      publish :fail, ["It's not your turn"]
      return
    end
    if @player.detective? && @player.game.detectives.map(&:current_node).detect { |node| node == @to_node }
