  ...
  describe '#finished?' do
    ...
    context "when the detectives haven't won" do
      ...
      context 'and the criminal has won' do
        before do
          expect(CriminalWonPolicy).to receive(:new).with(game: game).and_return instance_double('CriminalWonPolicy', won?: true)
        end
      ...
      context "and the criminal hasn't won" do
        before do
          expect(CriminalWonPolicy).to receive(:new).with(game: game).and_return instance_double('CriminalWonPolicy', won?: false)
        end
